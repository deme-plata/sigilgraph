...
fn submit(addr: &str, sub: &Submission) -> SubmitResult {
    ...
    serde_json::from_str(&body).unwrap_or(SubmitResult { accepted: false, reason: Some(format!("unparsed: {body}")), ..Default::default() })
}
...
fn main() {
    ...
    let forged = Challenge { height: future_h, vdf_input: guessed_seed, blake4_target: c.blake4_target, vdf_t: c.vdf_t, net_hps: 0.0, share_target: 0, share_vdf_t: 0 };
