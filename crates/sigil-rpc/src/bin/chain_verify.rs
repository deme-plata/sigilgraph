...
/// Independently verify one block against `my_tip`. Returns the new tip or an error.
fn verify_block(rec: &Value, my_tip: &[u8; 32], g: &ModSquaring) -> Result<[u8; 32], String> {
    ...
    if reward != sigil_emission::block_reward(bh) { return Err(format!("block {bh}: reward {reward} != schedule {}", sigil_emission::block_reward(bh))); }
...
fn main() {
    ...
    for bh in 0..head_bh {
        ...
        match verify_block(&rec, &tip, &g) {
            Ok(t) => { tip = t; verified += 1; }
    ...
    if head_bh > 0 {
        ...
        tamper_ok = verify_block(&rec0, &genesis(), &g).is_err();
