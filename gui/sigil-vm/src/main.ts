...
function renderRows(): void {
  ...
  swapWithFlash($('#moversRow'), snap.movers.map((m) => ui.moverCard(m, wallet)).join('') || '<div class="pempty">No miners read — node offline?</div>')
  $('#salesRow').innerHTML = snap.sales.map((x) => ui.saleCard(x, wallet)).join('') || '<div class="pempty">Nothing settled this week that the node reports.</div>'
...
document.addEventListener('click', (ev) => {
  ...
  if (btn.id === 'wipBtn') { openModal(`<h4>Work in progress</h4><p>SIGIL VM is being built in the open. The <b>design is done</b>; the <b>market underneath is not</b>. Every figure is chipped <span class="chip live">live</span> (read from the node this poll), <span class="chip derived">derived</span> (a stated calculation) or <span class="chip pretend">pretend</span> (no chain source yet). What is still pretend: prices, pools and quotes (no oracle, no pool), listings and sales (no listing exists), the cart, limit orders, the wSIGIL3/SSHARE rows, the novel.</p><p class="muted" style="font-size:12.5px">Read the handoff: <a class="mono" href="/downloads/sigil-vm-handoff.md" target="_blank" rel="noopener">sigil-vm-handoff.md ↗</a> · full inventory: <a class="mono" href="/downloads/sigil-vm-inventory.md" target="_blank" rel="noopener">sigil-vm-inventory.md ↗</a></p><div class="cm-foot" style="padding:12px 0 0;border:0"><button class="btn ghost" id="modalClose2">Close</button></div>`); return }
  ...
  if (ds.clearRecent) { ev.preventDefault(); ev.stopPropagation(); recent.splice(0); try { localStorage.removeItem('sigilvm-recent') } catch { /* */ } $('#q').focus(); globalSearch(($('#q') as HTMLInputElement).value); return }
  if (ds.shareTok) { ev.preventDefault(); const url = `${location.origin}${location.pathname}#tok=${ds.shareTok}`; navigator.clipboard?.writeText(url).then(() => toast(`Copied link — ${url.replace(location.origin, '')}`)).catch(() => toast('Clipboard blocked.', 'warn')); return }
...
document.querySelector('.search')?.addEventListener('focusout', (ev) => { const to = (ev as FocusEvent).relatedTarget as HTMLElement | null; if (!to && !document.contains(ev.target as Node)) return; /* Chrome fires focusout when a focused node is removed by a re-render — that is not the user leaving */ if (!to || !to.closest('.search')) $('#qres').classList.remove('open') })
...
document.addEventListener('click', (ev) => { const t = ev.target as HTMLElement; if (!t.isConnected) return; /* a control the click itself re-rendered away is not an outside click */ if (!t.closest('.search')) $('#qres').classList.remove('open'); if (!t.closest('.chain-wrap')) $('#chainMenu').classList.remove('open') })
...
function globalSearch(q: string): void {
  ...
  if (rc.length) rows.push('<div class="h">Recent <button class="hclr" data-clear-recent="1" title="Forget the recent list (this browser only)">clear</button></div>' + rc.map((c) => `<div class="r" data-coll="${c.id}"><img src="${c.cover}" alt=""><div><div class="n">${ui.esc(c.name)}</div><div class="s">${c.items === null ? '—' : fmt.int(c.items)} ${c.items === 1 ? 'item' : 'items'} · open</div></div></div>`).join(''))
