...
network expansion. The mapping is exact only for quadratic potentials — anti-windup,
...
conserved; a fork that changes the braid changes the invariant. Two constraints bind the
programme. \emph{Computational:} the Jones polynomial is \#P-hard in general, so the
...
setup). The variation yields the dispatch rule: retire a leg the instant its hardness
margin drops; the others carry what they guard. \textbf{Composition, stated carefully
...
$\SSIGIL$ admits a true extremum iff $\Gamma_{\mathrm{verify}} \ge
\Gamma_{\mathrm{state}}$. Measured 2026-05-30 at $\sim$1840:1 against verification (a
constrained saddle), resolved not by faster per-signature verification but by changing
