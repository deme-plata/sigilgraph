...
| Shareable URLs — `#coll=<id>` opens a collection sheet (tab title follows), `#tok=<symbol>` opens a token drawer; copy-link buttons in the sheet head and the drawer; on phones Back closes any sheet or the drawer (they are history entries, like OpenSea's routes) | — | the hash follows the sheet open/close |
