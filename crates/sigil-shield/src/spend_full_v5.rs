...
pub fn v5_options() -> ProofOptions {
...
/// fewer real outputs pads with zero-value notes (see [`build_spend_full_v5_trace`]).
...
#[derive(Clone)]
pub struct SpendFullV5PublicInputs {
    pub root: BaseElement,
    pub nf: BaseElement,
    pub fee: BaseElement,
    pub cm_outs: [BaseElement; N_OUTS],
}
impl ToElements<BaseElement> for SpendFullV5PublicInputs {
...
impl Air for SpendFullV5Air {
    type BaseField = BaseElement;
    type PublicInputs = SpendFullV5PublicInputs;
    type GkrProof = ();
    type GkrVerifier = ();

    fn new(trace_info: TraceInfo, pub_inputs: SpendFullV5PublicInputs, options: ProofOptions) -> Self {
...
pub struct SpendFullV5Prover {
    options: ProofOptions,
}
impl SpendFullV5Prover {
    pub fn new(options: ProofOptions) -> Self {
        Self { options }
    }
}
impl Prover for SpendFullV5Prover {
    type BaseField = BaseElement;
    type Air = SpendFullV5Air;
    ...
    fn get_pub_inputs(&self, trace: &Self::Trace) -> SpendFullV5PublicInputs {
...
/// and never acceptable in a wallet. [`build_spend_full_v5_trace`] wraps this with OS entropy.
pub fn build_spend_full_v5_trace_seeded(
    value: BaseElement,
    ...
    assert!(
        ...
        "spend_full_v5 requires depth+1 a power of two (depth 1, 3, 7, 15, …); got depth {depth}"
    );
...
/// [`build_spend_full_v5_trace`], which draws from the OS.
...
pub fn build_spend_full_v5_trace(
    value: BaseElement,
    blinding: BaseElement,
    spend_key: BaseElement,
    fee: BaseElement,
    ...
    build_spend_full_v5_trace_seeded(value, blinding, spend_key, fee, outs, path, options, seed)
}
...
impl HidingProver for SpendFullV5Prover {
...
pub fn verify_spend_full_v5(
    proof: winterfell::Proof,
    pub_inputs: SpendFullV5PublicInputs,
) -> Result<(), winterfell::VerifierError> {
    let min = winterfell::AcceptableOptions::MinConjecturedSecurity(ACCEPT_BITS);
    winterfell::verify::<SpendFullV5Air, Blake3_256<BaseElement>, Coin>(proof, pub_inputs, &min)
}
...
mod tests {
    ...
    fn owner_bound_outputs_verify_without_naming_the_recipient() {
        ...
        let trace = build_spend_full_v5_trace(value, blinding, sk, fee, &outs, &path, &v5_options());
        let proof = SpendFullV5Prover::new(v5_options()).prove(trace).expect("prove");
        let pub_in = SpendFullV5PublicInputs {
            root: tree.root(),
        ...
        verify_spend_full_v5(proof, pub_in).expect("an honest owner-bound spend must verify");
    }
    ...
    fn a_foreign_key_cannot_spend_a_known_note() {
        ...
        let verdict = match std::panic::catch_unwind(|| {
            SpendFullV5Prover::new(v5_options()).prove(trace)
        }) {
            Err(_) | Ok(Err(_)) => Err(()),
            Ok(Ok(p)) => verify_spend_full_v5(p, SpendFullV5PublicInputs {
                root: tree.root(),
    ...
    fn unowned_output_commitment_is_rejected() {
        ...
        let trace = build_spend_full_v5_trace(value, blinding, sk, e(3), &outs, &path, &v5_options());
        let proof = SpendFullV5Prover::new(v5_options()).prove(trace).expect("prove");
        // v3-shaped (un-owned) output commitments
        let v3_shape = [compress2(e(50), e(777)), compress2(e(47), e(888))];
        assert!(
            verify_spend_full_v5(proof, SpendFullV5PublicInputs {
                root: tree.root(),
                nf: compress2(sk, BaseElement::new(pos as u64)),
                fee: e(3),
                cm_outs: v3_shape,
    ...
    fn root_conservation_and_range_still_enforced() {
        ...
        let trace = build_spend_full_v5_trace(value, blinding, sk, e(3), &outs, &path, &v5_options());
        let proof = SpendFullV5Prover::new(v5_options()).prove(trace).expect("prove");
        assert!(verify_spend_full_v5(proof, SpendFullV5PublicInputs {
            root: e(0xBADC0FFEE),
            nf: compress2(sk, BaseElement::new(pos as u64)),
            fee: e(3),
        ...
        assert!(std::panic::catch_unwind(|| {
            build_spend_full_v5_trace(value, blinding, sk, e(3),
                &[(e(50), e(777), me), (e(48), e(888), me)], &path, &v5_options())
        }).is_err(), "non-conserving must be refused");
        let over = BaseElement::new(1u64 << RANGE_BITS);
        assert!(std::panic::catch_unwind(|| {
            build_spend_full_v5_trace(value, blinding, sk, e(3),
                &[(over, e(777), me), (e(0), e(888), me)], &path, &v5_options())
        }).is_err(), "out-of-range must be refused");
    }

    /// THE GATE. v4's proof carried the recipient key and both output amounts verbatim,
    /// 85 times each. This asserts v5's does not carry ANY of them — and it drives the real
    /// production path, `HidingProver::prove_hiding`, so it fails the same way a wallet would.
    ///
    ...
    fn zk_proof_does_not_contain_its_own_witness() {
        ...
        let prover = SpendFullV5Prover::new(v5_options());
        let trace = build_spend_full_v5_trace(value, blinding, sk, fee, &outs, &path, &v5_options());
        // prove_hiding itself refuses to return a leaking proof — so reaching Ok IS the assertion.
        let proof = prover.prove_hiding(trace).expect("a hiding proof must be produced");

        // Belt and braces: scan again here, so the test fails loudly rather than silently
        // depending on prove_hiding having been wired up.
        let leaks = scan_proof_for_secrets(
            &proof.to_bytes(),
            &[("recipient pk", bob), ("output A", e(50)), ("output B", e(47)),
              ("sender pk", me), ("spend key", sk), ("input value", value),
              ("input blinding", blinding)],
        );
        assert!(leaks.is_empty(), "witness leaked into the proof: {leaks:?}");

        // And it must still be a VALID proof — hiding is worthless if it stops verifying.
        let pub_in = SpendFullV5PublicInputs {
            root: tree.root(),
            nf: compress2(sk, BaseElement::new(pos as u64)),
            fee,
        ...
        verify_spend_full_v5(proof, pub_in).expect("a hiding proof must still verify");
    }
    ...
    fn two_proofs_of_the_same_spend_differ() {
        ...
        let o = v5_options();
        let mk = |seed: [u8; 32]| {
            let t = build_spend_full_v5_trace_seeded(value, blinding, sk, e(3), &outs, &path, &o, seed);
            SpendFullV5Prover::new(o.clone()).prove(t).expect("prove").to_bytes()
        };
        assert_ne!(mk([1u8; 32]), mk([2u8; 32]), "different mask seeds must give different proofs");
        assert_eq!(mk([1u8; 32]), mk([1u8; 32]), "the same seed must be reproducible");
    }

    /// The AIR must reserve strictly more randomness than the verifier opens, at every query
    /// count we might ship. Pure arithmetic — runs in debug, no prover involved.
    #[test]
    fn reserved_randomness_outruns_every_shippable_query_count() {
        use crate::zk_mask::{exemptions_for, padded_len, zk_rows_for};
        use winterfell::FieldExtension;
        for q in [21usize, 32, 42, 64, 84] {
            let o = ProofOptions::new(q, 8, 16, FieldExtension::Quadratic, 8, 31);
            let real = 4 * SEG; // depth 3, the shape the pool actually uses
            let n = padded_len(real, &o);
            assert_eq!(n, real * 2, "v5's AIR derives the real region as trace_len/2");
            assert!(n - real > zk_rows_for(&o) - 1, "q={q}: not enough reserved rows");
            assert!(exemptions_for(n, real) <= n / 2 + 1, "q={q}: exceeds winterfell's exemption cap");
        }
    }
}
