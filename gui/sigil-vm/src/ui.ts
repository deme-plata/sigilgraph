...
export function strip(s: Snapshot): string {
  ...
  return [
    ...
    cell('Supply', isFinite(h.supplySigil) ? fmt.num(h.supplySigil) + ' SIGIL' : isFinite(h.transparentSigil) ? fmt.num(h.transparentSigil) + ' transparent' : '—', isFinite(h.mintedPct) ? `${h.mintedPct.toFixed(2)}% of 21M · ${fmt.num(h.transparentSigil)} open + ${fmt.num(h.valueLocked)} sealed` : isFinite(h.transparentSigil) ? 'shielded pool unread — minted total needs both' : 'supply unread this poll', 'rigs'),
...
export function forYou(s: Snapshot): string {
  ...
  return `<div class="foryou">
    <div class="fy-col">
      ...
      <p><b>What is it worth?</b> ${isFinite(h.supplySigil) ? `${fmt.num(h.supplySigil)} of 21M SIGIL minted (${h.mintedPct.toFixed(2)}%) — ${fmt.num(h.transparentSigil)} in open wallets, ${fmt.num(h.valueLocked)} sealed in the shielded pool.` : isFinite(h.transparentSigil) ? `${fmt.num(h.transparentSigil)} SIGIL in open wallets; the shielded pool did not answer, so the minted total is unknown this poll.` : 'Supply unread this poll — the supply route did not answer.'} There is <b>no on-chain price</b>: the USDS oracle is not fed and no SIGIL pool exists on g2. The only market is the wSIGIL3 pool on Polygon, off this chain. ${tag('MEASURED', 'live')} ${tag('EXTERNAL', 'derived')}</p>
