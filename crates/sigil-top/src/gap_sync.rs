...
mod tests {
    ...
    /// (ii-b) A genuinely corrupt frontier header is rejected at ingest and then fails loud.
    #[test]
    fn parent_mismatch_is_fatal_and_named() {
        ...
        let net = FakeNet { peers: vec![FakePeer::full(&chain)], tip_hint: N, lie_factor: 1 };
        let mut eng = GapSyncEngine::new(fresh_store("forge"), net).with_watchdog_ticks(2);
        eng.run(N + 1, 100, 8);
        let f = eng.failure.as_ref().expect("a forged parent link MUST fail loud");
        assert_eq!(f.height, 1000, "names the exact break height");
        assert!(f.reason.contains("hole") || f.reason.contains("watchdog"), "reason identifies the rejected frontier: {}", f.reason);
    }
