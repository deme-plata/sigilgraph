...
try {
  ...
  html = await page.evaluate(() => {
    ...
    const cards = (sel) => [...document.querySelectorAll(sel)].map((c) => `<li><b>${esc(t(c.querySelector('.name')))}</b> ${esc(t(c.querySelector('.byline, .when')))} — ${esc(t(c.querySelector('.kv')))}</li>`).join('')
