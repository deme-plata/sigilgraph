  ...
  <main class="main" id="main">
    ...
    <footer class="foot">
      <div class="foot-top">
        ...
        <div class="foot-cols">
          <div><h4>Market</h4><a href="#featured">Collections</a><a href="#drops">Drops</a><a href="#trending">Trending</a><a href="#swap">Swap</a><a href="/sigil-dex.html" target="_blank" rel="noopener">Trading desk</a></div>
          <div><h4>Chain</h4><a href="/sigil-explorer.html" target="_blank" rel="noopener">Explorer</a><a href="/api.html" target="_blank" rel="noopener">API</a><a href="/datacenter.html" target="_blank" rel="noopener">Datacenter</a><a href="/kristensen-board.html" target="_blank" rel="noopener">K board</a><a href="/kristensen-time.html" target="_blank" rel="noopener">Kristensen Time</a></div>
          <div><h4>Nation</h4><a href="/sigil-nation-whitepaper.html" target="_blank" rel="noopener">Whitepaper</a><a href="/sigil-wallet-tron-embedded.html#court" target="_blank" rel="noopener">Supreme Court</a><a href="/kristensen-earth.html" target="_blank" rel="noopener">Earth K⊕</a><a href="/bridge-slider.html" target="_blank" rel="noopener">Bridge</a></div>
          <div><h4>Build</h4><a href="/downloads/sigil-vm-src.tar.gz">Page source (.tar.gz)</a><a href="https://github.com/deme-plata/sigilgraph" target="_blank" rel="noopener">GitHub</a><a href="/sigil-top-openapi.json" target="_blank" rel="noopener">OpenAPI</a><a href="/flux-versions.html" target="_blank" rel="noopener">Flux versions</a></div>
        </div>
      </div>
...
export function strip(s: Snapshot): string {
  const h = s.head
  // every strip cell names its route in a tooltip — the figure's provenance, one hover away
  const cell = (k: string, v: string, small = '', coll = '', extra = '', title = '') => `<button class="cell"${coll ? ` data-coll="${coll}"` : ''}${title ? ` title="${esc(title)}"` : ''}><div class="k">${k}</div><div class="v"><span class="vt">${v}</span>${extra}</div>${small ? `<div class="s">${small}</div>` : ''}</button>`
  return [
    cell('Height', fmt.int(h.height), h.blkPerSec ? `${h.blkPerSec.toFixed(1)} blk/s` : 'rate after next poll', 'blocks', '', '/v1/mining/miners height this poll · blk/s = blocks counted between two polls'),
    cell('Finality', h.finalityGate, isFinite(h.committee) ? `h ${fmt.int(h.finalityHeight)} · ${fmt.n(h.committee, 'validator')}` : 'certificate unread this poll', 'blocks', '', '/v1/finality/certificate — gate, certified height, committee size'),
    cell('Supply', isFinite(h.supplySigil) ? fmt.num(h.supplySigil) + ' SIGIL' : isFinite(h.transparentSigil) ? fmt.num(h.transparentSigil) + ' transparent' : '—', isFinite(h.mintedPct) ? `${h.mintedPct.toFixed(2)}% of 21M in circulation · ${fmt.num(h.transparentSigil)} open + ${fmt.num(h.valueLocked)} sealed` : isFinite(h.transparentSigil) ? 'shielded pool unread — the circulating total needs both' : 'supply unread this poll', 'rigs', '', '/v1/supply native_supply (open wallets) + /v1/shielded/anchor value_locked (sealed) · shielding moves coins from the first to the second'),
    cell('Hashrate', fmt.hps(h.netHps), `${fmt.n(h.liveMiners, 'miner')}${h.hashChange !== null ? ' · ' + fmt.pct(h.hashChange) : s.hashHist.length ? '' : ' · history unread this poll'}`, 'rigs', s.hashHist.length > 2 ? sparkline(s.hashHist, 90, 22, h.hashChange !== null && h.hashChange < 0 ? 'down' : 'up') : '', '/v1/mining/miners net_hps + live miners · change = first vs last point of /v1/mining/hashrate/history'),
    cell('Shielded pool', isFinite(h.notes) ? fmt.int(h.notes) + ' notes' : '—', isFinite(h.notes) ? `${fmt.num(h.valueLocked)} SIGIL locked · ${fmt.int(h.nullifiers)} spent` : 'anchor unread this poll', 'notes', '', '/v1/shielded/anchor — notes in the pool, value locked, nullifiers spent'),
    cell('Treasury', isFinite(h.treasurySigil) ? fmt.num(h.treasurySigil) + ' SIGIL' : '—', isFinite(h.treasurySigil) ? 'nation welfare' : 'nation route unread this poll', 'treasury', '', '/v1/nation/status treasury_glyphs'),
  ].join('')
}
...
export function forYou(s: Snapshot): string {
  ...
  return `<div class="foryou">
    <div class="fy-col">
      <h2 class="fy-h">For you <span class="muted">— money words, not instruments</span></h2>
      <p><b>Is my payment final?</b> ${h.ok && !isFinite(h.lagBlocks) ? 'No finality certificate was read this poll — the finality route did not answer, so this page cannot say.' : h.ok ? `The finality certificate sits ${fmt.int(h.lagBlocks)} block${h.lagBlocks === 1 ? '' : 's'} behind the tip${lagS !== null ? ` ≈ <b>${lagS < 1 ? '<1' : lagS.toFixed(0)} s</b>` : ''} (gate <span class="mono">${esc(h.finalityGate)}</span>, ${fmt.n(h.committee, 'validator')}).` : 'Node unreachable — no certificate read.'} ${tag('MEASURED', 'live')}</p>
      <p><b>What protects it?</b> ${isFinite(h.netHps) ? `${fmt.int(h.liveMiners)} live rigs at ${fmt.hps(h.netHps)} (BLAKE4 + VDF)` : 'Rigs and hashrate unread this poll — the mining route did not answer'}, ${isFinite(h.notes) ? `${fmt.int(h.notes)} sealed notes holding ${fmt.num(h.valueLocked)} SIGIL` : 'a shielded pool whose anchor was unread this poll'}. ${tag('MEASURED', 'live')} The producer also runs a hybrid post-quantum (SQIsign + Ed25519) block check — in observe-only mode, and no route exposes its verdicts yet, so this page cannot measure it. ${tag('DESIGN', 'gold')}</p>
      <p><b>What is it worth?</b> ${isFinite(h.supplySigil) ? `${fmt.num(h.supplySigil)} SIGIL in circulation (${h.mintedPct.toFixed(2)}% of the 21M cap) — ${fmt.num(h.transparentSigil)} in open wallets, ${fmt.num(h.valueLocked)} sealed in the shielded pool.` : isFinite(h.transparentSigil) ? `${fmt.num(h.transparentSigil)} SIGIL in open wallets; the shielded pool did not answer, so the circulating total is unknown this poll.` : 'Supply unread this poll — the supply route did not answer.'} There is <b>no on-chain price</b>: the USDS oracle is not fed and no SIGIL pool exists on g2. The only market is the wSIGIL3 pool on Polygon, off this chain. ${tag('MEASURED', 'live')} ${tag('EXTERNAL', 'derived')}</p>
      <p><b>Can I pay with it?</b> Yes — privately only. Transparent sends are retired, so every payment is a sealed note. ${isFinite(h.notes) ? `${h.capacity ? `<b>${fmt.int(Math.max(0, h.capacity - h.notes))}</b> of ${fmt.int(h.capacity)} leaves are free this epoch, ` : ''}${fmt.n(h.registered, 'wallet has', 'wallets have')} published receiving keys, and ${fmt.n(h.nullifiers, 'note has', 'notes have')} ever been spent.` : 'The pool anchor was unread this poll, so its free leaves and spent notes cannot be stated.'} The proof and the signature happen in your wallet app, never on this page. ${tag('MEASURED', 'live')}</p>
    </div>
    <div class="fy-col">
      <h2 class="fy-h">What's next <span class="muted">— ETAs from live height and block rate</span></h2>
...
export function moverCard(m: Mover, mine: string | null = null): string {
  const you = !!mine && m.id.toLowerCase() === mine // a mover's id is its rig's wallet
  return `<div class="card mover${m.idle ? ' idle' : ''}${you ? ' you' : ''}" title="${esc(m.id)}" data-coll="rigs" role="button" tabindex="0">
    <div class="img" style="background-image:url('${m.cover}')"><span class="prov">${prov(m.provenance)}${you ? '<span class="chip live yours"><i class="d"></i>yours</span>' : ''}</span>${m.idle ? '<span class="status chip"><i class="d"></i>idle</span>' : m.sub.startsWith('GPU') ? '<span class="status chip derived"><i class="d"></i>GPU</span>' : ''}<span class="cta"><span class="cta-blurb">${m.idle ? 'idle · last seen ' : ''}${esc(m.sub)}<br><span class="mono">wallet ${fmt.short(m.id, 8)}</span></span><span class="cta-btn">Open rigs ↗</span></span></div>
...
export function panelHead(addr: string | null, s: Snapshot, balances: Record<string, number>): string {
  ...
  const nativeRead = balances['0'.repeat(64)] !== undefined
  const native = balances['0'.repeat(64)] ?? NaN // unread → dash, never a 0 SIGIL portfolio
  ...
  return `<div class="who"><img src="${avatarSvg(addr, addr.slice(0, 1).toUpperCase())}" alt=""><div><div class="a" title="${esc(addr)}">${fmt.short(addr, 6)} <button class="copy" data-copy="${esc(addr)}" title="Copy wallet id">${I.copy}</button></div><div class="b">sigil-g2 · ${s.head.ok ? 'live' : 'offline'}${rig ? ` · mining as ${esc(rig.rig)}` : ''} · <button class="forget" id="pDisconnect" title="Forget this wallet on this page (nothing on chain changes)">forget</button></div></div><button class="ibtn x" id="panelClose" title="Close panel" aria-label="Close panel">${I.x}</button></div>
    <div class="total"><div class="k">Portfolio</div><div class="v">${nativeRead ? native.toLocaleString('en-US', { maximumFractionDigits: 4 }) : '—'}<small>SIGIL</small></div><div class="muted" style="font-size:11.5px">${!nativeRead ? 'balance unread this poll — the balance route did not answer · ' : glyphs !== null ? fmt.int(glyphs) + ' glyphs · ' : ''}USD value unavailable — no oracle price on chain yet</div></div>`
}
...
export function panelActivity(s: Snapshot, mine: string | null = null): string {
  ...
  if (s.head.ok) row(I.swap, '', 'Finality certificate', `${s.head.finalityGate} · height ${fmt.int(s.head.finalityHeight)} · ${fmt.n(s.head.committee, 'validator')}`, `${fmt.int(s.head.lagBlocks)} blk`, 'blocks')
...
export function collectionModal(c: Collection, s: Snapshot, watched = false, mine: string | null = null): string {
  ...
  return `<div class="cm-head" style="background-image:url('${c.cover}')"><button class="ibtn share" data-share="${c.id}" aria-label="Copy a link to this collection" title="Copy link — sigil-vm.html#coll=${c.id}">${I.link}</button><button class="ibtn x" id="modalClose" aria-label="Close" title="Close (Esc)">${I.x}</button></div>
