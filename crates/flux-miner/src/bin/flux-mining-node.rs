            ...
            let result = match serde_json::from_str::<Submission>(&body) {
                Ok(sub) => {
                    ...
                    if check_submission(&g, &c, &sub) {
                        ...
                        SubmitResult { accepted: true, reason: None, ..Default::default() }
                    } else {
                        println!("  ✗ rejected h={} from {}", sub.height, sub.wallet);
                        SubmitResult { accepted: false, reason: Some("dual-lane verify / height / header mismatch".into()), ..Default::default() }
                    }
                }
                Err(e) => SubmitResult { accepted: false, reason: Some(format!("bad submission json: {e}")), ..Default::default() },
            };
