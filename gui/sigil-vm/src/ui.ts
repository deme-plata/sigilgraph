  ...
  return `<div class="cm-head" style="background-image:url('${c.cover}')"><button class="ibtn x" id="modalClose">${I.x}</button></div>
    ...
    <div class="cm-body">
      ...
      <div class="cm-pane" data-pane="info" hidden><p class="blurb">${esc(c.blurb)}</p><div class="td-grid">
        ...
        <div class="td-cell wide"><div class="k">Last poll</div><div class="v mono" style="white-space:nowrap">${new Date(s.at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })} · h ${fmt.int(s.head.height)}</div></div>
      </div></div>
    </div>
