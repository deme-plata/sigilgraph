...
export function strip(s: Snapshot): string {
  ...
  return [
    ...
    cell('Hashrate', fmt.hps(h.netHps), `${fmt.n(h.liveMiners, 'miner')}${h.hashChange !== null ? ' · ' + fmt.pct(h.hashChange) : ''}`, 'rigs', s.hashHist.length > 2 ? sparkline(s.hashHist, 90, 22, h.hashChange !== null && h.hashChange < 0 ? 'down' : 'up') : '', '/v1/mining/miners net_hps + live miners · change = first vs last point of /v1/mining/hashrate/history'),
