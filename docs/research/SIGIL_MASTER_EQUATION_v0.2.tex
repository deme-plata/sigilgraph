...
\tcbuselibrary{skins}
% layman explainer box — same design system as the whitepaper
...
on the same machine. We have \emph{not} proven the machine works this way. But designing
\emph{as if} it does keeps every piece pulling in the same direction — and it hands us
...
the ink — one already has (Prediction 7, the delivery law).}
...
\layman{sigilcyan}{the wall}{We assumed verification speed was a hardware problem —
buy faster machines, verify faster. It turned out to be a \emph{choice-of-mathematics}
problem: switch the proof system, and checking the whole chain becomes one
constant-size receipt instead of a million re-computations. The wall didn't get
climbed. It got deleted — exactly where the security term of the action said the
pressure was.}
